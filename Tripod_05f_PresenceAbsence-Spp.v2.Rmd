...
hl <- hoslem.test(PAdata$laoc_PA, fitted(model.bin.step), g=5)
cbind(hl$observed, hl$expected)
...
# Goodness of fit using Hosmer-Lemeshow: comparison of observed 1s & 0s vs. predicted
# Set # groups g to > # covariates + 1
hl <- hoslem.test(PAdata$pien_PA, fitted(model.bin.step), g=6)
cbind(hl$observed, hl$expected)
...
# Goodness of fit using Hosmer-Lemeshow: comparison of observed 1s & 0s vs. predicted
# Set # groups g to > # covariates + 1
hl <- hoslem.test(PAdata$pipo_PA, fitted(model.bin.step), g=6)
cbind(hl$observed, hl$expected)
...
# Goodness of fit using Hosmer-Lemeshow: comparison of observed 1s & 0s vs. predicted
# Set # groups g to > # covariates + 1
hl <- hoslem.test(PAdata$psme_PA, fitted(model.bin.step), g=4)
cbind(hl$observed, hl$expected)
...
Moran_space(data, model.bin.step$residuals)
# Null is no SAC

# semivariogram
data$x <- data$long
data$y <- data$lat
temp <- data.frame(x=data$x, y=data$y, resids=model.bin.step$residuals)
...
### 180206: ALL OF THESE PLOT GENERATORS HAVE BEEN CHANGED
...
# Pick most significant topo predictors for laoc
sjp.glm(model.PA.laoc, type = "slope", facet.grid = FALSE, show.ci = TRUE, vars = "dem100")

# Pick most significant topo predictors for pien
sjp.glm(model.PA.pien, type = "slope", facet.grid = FALSE, show.ci = TRUE, vars = "dem100")
sjp.glm(model.PA.pien, type = "slope", facet.grid = FALSE, show.ci = TRUE, vars = "cti100")

# Pick most significant topo predictors for pipo
sjp.glm(model.PA.pipo, type = "slope", facet.grid = FALSE, show.ci = TRUE, vars = "dem100")
sjp.glm(model.PA.pipo, type = "slope", facet.grid = FALSE, show.ci = TRUE, vars = "slp100")
sjp.glm(model.PA.pipo, type = "slope", facet.grid = FALSE, show.ci = TRUE, vars = "tpi100")

# No significant topo predictors for psme.
# All of the above plots have pretty lousy curves except for DEM & LAOC.

http://www.theanalysisfactor.com/r-glm-plotting/
```
